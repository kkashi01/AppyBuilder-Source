...
public final class MockForm extends MockContainer {
    ...
    // Property names
    private static final String PROPERTY_NAME_ACTION_BAR = "ActionBarColor";
    private static final String PROPERTY_NAME_TITLE = "Title";
    private static final String PROPERTY_NAME_TITLE_COLOR = "TitleColor";
    ...
    public MockForm(SimpleEditor editor) {
        ...
        phoneWidget.setStylePrimaryName("ode-SimpleMockFormPhoneVertical");
...
//    initComponent(formWidget);
    ...
    public void changePreviewSize(boolean isTablet) {
        ...
        if (landscape) {
            if (isTablet) {
                phoneWidget.setStylePrimaryName("ode-SimpleMockFormPhoneHorizontalTablet");
            } else {
                phoneWidget.setStylePrimaryName("ode-SimpleMockFormPhoneHorizontal");
            }
            resizePanel(LANDSCAPE_WIDTH, LANDSCAPE_HEIGHT);
        } else {
            if (isTablet) {
                phoneWidget.setStylePrimaryName("ode-SimpleMockFormPhoneVerticalTablet");
            } else {
                phoneWidget.setStylePrimaryName("ode-SimpleMockFormPhoneVertical");
            }
    ...
    */
    ...
    protected boolean isPropertyVisible(String propertyName) {
        ...
        if (propertyName.equals(PROPERTY_NAME_TUTORIAL_URL)
                || propertyName.equals(SettingsConstants.YOUNG_ANDROID_SETTINGS_MinAPI)
                || propertyName.equals(SettingsConstants.YOUNG_ANDROID_SETTINGS_MaxAPI)
                ) {
            // These properties actually applies to the application and is only visible on Screen1.
            return editor.isScreen1();
        }

        if (propertyName.equals(PROPERTY_NAME_APPLICATION_PACKAGE)) {
            // Only visible on Screen1.
            return editor.isScreen1();
        }
...
//    private void setForegroundColorProperty(String text) {
//        MockComponentsUtil.setWidgetForegroundColor(rootPanel, text);
//    }

    private void setBackgroundColorProperty2(Widget widget, String text) {
        if (MockComponentsUtil.isNoneColor(text)) {
            text = "&HFF000000";  // black
        } else if (MockComponentsUtil.isDefaultColor(text)) {
            text = "&HFFFFFFFF";  // white
    ...
    private void setScreenOrientationProperty(String text) {
        if (hasProperty(PROPERTY_NAME_WIDTH) && hasProperty(PROPERTY_NAME_HEIGHT) &&
            ...
            if (text.equalsIgnoreCase("landscape")) {
                ...
                landscape = true;
                phoneWidget.setStyleName("ode-SimpleMockFormPhoneHorizontal");
            } else {
                screenWidth = PORTRAIT_WIDTH;
                screenHeight = PORTRAIT_HEIGHT;
                phoneWidget.setStyleName("ode-SimpleMockFormPhoneVertical");
                landscape = false;
            }
    ...
    private void setMaxAPIProperty(String value) {
        if (editor.isScreen1()) {
            editor.getProjectEditor().changeProjectSettingsProperty(
                    SettingsConstants.PROJECT_YOUNG_ANDROID_SETTINGS,
                    SettingsConstants.YOUNG_ANDROID_SETTINGS_MaxAPI, value);
        }
    }
...
//        } else if (propertyName.equals(PROPERTY_NAME_TITLE_COLOR)) {
//            titleBar.changeTitleColor( "#" + MockComponentsUtil.getHexString(newValue, 6));
        } else if (propertyName.equals(PROPERTY_NAME_ACTION_BAR)) {
            setBackgroundColorProperty2(titleBar, newValue);
        ...
        } else if (propertyName.equals(SettingsConstants.YOUNG_ANDROID_SETTINGS_MaxAPI)) {
            setMaxAPIProperty(newValue);
