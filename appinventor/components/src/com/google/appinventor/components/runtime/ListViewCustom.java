...
//    private static final int DEFAULT_TEXT_SIZE = 22;
  ...
  /**
  * Sets the height of the view on the screen
  ...
  */
  ...
  /**
  * Sets the width of the view on the screen
...
//      String itemString = items.getString(i).toString(); // used if we use YailList as Array (new version of java bridge)
      ...
      if (!container.$form().getCompatibilityMode()) {
        displayTextSize = (textSize * container.$form().deviceDensity());
      }
      chars.setSpan(new AbsoluteSizeSpan((int) displayTextSize),0,chars.length(),0);
      objects[i - 1] = chars;
    }
    return objects;
  }
...
//            isSwipeToArchive=true;
    ...
    private void setScaleAdapter() {
        ...
        animAdapter.setAbsListView(view);
        view.setAdapter(animAdapter);
    }
    ...
    private void setSwipeDismissAdapter() {
        ...
        adapter.setAbsListView(view);
        view.setAdapter(adapter);
    }


    @Override
    public void onDismiss(final android.widget.AbsListView view, final int[] reverseSortedPositions) {
        String deletedItem="";
...
// do we want to show text or make it invisible?
                if (!showText) {
                    ...
                    viewHolder.txtDesc.setTextSize(TextSize());
                }
...
//                viewHolder.imageView.setImageDrawable(MediaUtil.getBitmapDrawable(mContext , imageName));
                ...
                if (imageName.startsWith("/") ) {
                    Log.d(LOG_TAG, "Image starts with path slash");
                    String externalStoragePath = Environment.getExternalStorageDirectory().getPath();
                    Log.d(LOG_TAG, "external path is:" + externalStoragePath);
                    java.io.File imgFile = new java.io.File( externalStoragePath + imageName);
                    Log.d(LOG_TAG, "after file");
                    if(imgFile.exists()){
                        ...
                        Log.d(LOG_TAG, "Image does not exists");
