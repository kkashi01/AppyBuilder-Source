...
// Should merge in these changes: https://github.com/mit-cml/appinventor-sources/blob/master/appinventor/components/src/com/google/appinventor/components/runtime/ButtonBase.java
...
public abstract class ButtonBase extends AndroidViewComponent
 ...
 @SimpleFunction( description = "Simulates user's click on the component")
    ...
    if (backgroundImageDrawable == null) {
      if (shape == Component.BUTTON_SHAPE_DEFAULT) {
        if (backgroundColor == Component.COLOR_DEFAULT) {
          ...
          view.setBackground(defaultButtonDrawable);
        } else {
            // Now we check if the button shape is default and
            // the user want a transparent background color for the button
            if (backgroundColor == Component.COLOR_NONE || backgroundColor == 16777215) {
                // clear the background image
                view.setBackground(null);
                // Set to the specificed color (possibly COLOR_NONE for transparent)
                view.setBackground(defaultButtonDrawable);
  ...
  public void FontTypefaceCustom(String fontFileName) {
    if (fontFileName == null || TextUtils.isEmpty(fontFileName)) {
      return;
    }
