...
//    @SimpleProperty(category = PropertyCategory.BEHAVIOR, description = "Connects to FTP server. You should have already set ServerURL, ServerUserId, ServerPassword. ")
    private boolean connectToFTPServer() {
        ...
        try {
            ...
            if (!isConnected) {
                FtpConnectionError(String.format("Unable to connect to serverURL %s, user serverUserId of %s. Ensure userid/password are correct ",
                        serverURL, serverUserId));
            } else {
...
//        localFileName = "/data/foo.ks";
    ...
    // For local can use root like /folder1/folder2/folder3
    // for remote can use /public_html/folder1/folder2/
   @SimpleFunction(description = "Deletes a remote or local directory. Will not delete if directory contains data.")
    public void DeleteFolder(final String fullPath, final boolean isRemote) {
        AsynchUtil.runAsynchronously(new Runnable() {
            @Override
            public void run() {
                asyncDeleteDir(fullPath, isRemote);
            }
        });
    }
...
//        remoteFile="*";
    ...
    private void asyncMakeDir(String fullPath, String newDirName, boolean isRemote) {
        ...
        try {
            ...
            AfterAction(false, "FTP error occurred when making new directory. Error: " + ex.getLocalizedMessage(), "FtpMakeDir");
        }
    }

    private void asyncDeleteDir(String fullPath, boolean isRemote) {
        ...
        if (!isRemote) {
            ...
            File file = new File(externalStoragePath + fullPath);
            if (file.exists() && file.isDirectory() ) {
                String[] listOfFiles = file.list();
                if (listOfFiles != null ) {
                    if (listOfFiles.length > 0) {
                        // non-empty, we don't delete
                        AfterAction(false, "Folder(s) contains files " + fullPath, "FtpDeleteDir");
                        return;
                    } else {
                       file.delete();
                        AfterAction(false, "Deleted fodler: " + fullPath, "FtpDeleteDir");
                        return;
                    }
                } else {
                    AfterAction(false, "Unable to determine directory status " + fullPath, "FtpDeleteDir");
                    return;
                }

            } else {
                AfterAction(false, "Specified path is not a directory or does not exist: " + fullPath, "FtpDeleteDir");
                return;
            }
        }
        ...
        try {
            ...
            boolean success = ftpClient.changeWorkingDirectory(fullPath);
            if (!success) {
                AfterAction(false, "fullPath is invalid: " + fullPath, "FtpDeleteDir");
                return;
            }

            success = ftpClient.removeDirectory(fullPath);
            if (success) {
                AfterAction(true, "Directory was deleted: " + fullPath, "FtpDeleteDir");
            } else {
                AfterAction(false, "Unable to delete directory. Either invalid or contains files: " + fullPath, "FtpDeleteDir");
            }
        } catch (Exception ex) {
            AfterAction(false, "FTP error occurred when deleting directory. Error: " + ex.getLocalizedMessage(), "FtpDeleteDir");
        }
    }
