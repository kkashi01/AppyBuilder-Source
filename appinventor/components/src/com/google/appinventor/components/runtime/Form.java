...
@DesignerComponent(version = YaVersion.FORM_COMPONENT_VERSION,
        ...
        showOnPalette = false)
@SimpleObject
@UsesLibraries(libraries = "appybuilder-billing3-1.0.32.jar,styledtoast.aar,fancydialog.aar" +
        ",appcompat-v7.aar, support-v4.aar" +
        ", animated-vector-drawable.aar" +
...
//        +"\nYou'll need to relaunch the application to use it again."
    ...
    private int APPY_BUILDER_ID_PERCENT = 5;  // 5 percent
    private int statusBarColor;
    private static final int DEFAULT_BACKGROUND_COLOR = Component.COLOR_BLACK;
    private int titleColor = Component.COLOR_WHITE;
...
//    Log.d(LOG_TAG, "xxxxx usercheck: user email is: " + userEmail);
    ...
    private void defaultPropertyValues() {
        ...
        ActionBarColor(Component.COLOR_INDIGO);
        Title(title);
        ShowStatusBar(true);
        TitleVisible(true);
...
// This code is not really a "fix" but more of a "workaround."
    ...
    public void ErrorOccurredDialog(Component component, String functionName, int errorNumber,
        ...
        if ((!(EventDispatcher.dispatchEvent(
            ...
            // The app writer can override this by providing an error handler.
            new Notifier(this).ShowMessageDialog("Error " + errorNumber + ": " + message, title, buttonText, 1);
        }
    }
    ...
    private void recomputeLayout() {
        ...
        // recreates the padding based on whatever that user had initially set
        Padding(Padding());
    ...
    @SimpleProperty(description = "The padding around the screen. Use single number like 5 to specify padding for top, left, bottom, righ. " +
            "You can also use 4 different numbers like 5,0,10,0 for top, left, bottom right.", category = PropertyCategory.BEHAVIOR)
    public void Padding(String padding) {
        padding = padding.trim();
        if (padding.isEmpty()) return;

        String sizes[] = padding.split(",");

        this.padding = padding;

        List<Integer> actualSizes = new ArrayList<Integer>();
        for (int i = 0; i < sizes.length; i++) {
            int size = convertDpToDensity(Integer.valueOf(sizes[i].trim()));
            actualSizes.add(size);
        }

        if (sizes.length == 1) {
            frameLayout.setPadding(actualSizes.get(1 - 1), actualSizes.get(1 - 1), actualSizes.get(1 - 1), actualSizes.get(1 - 1));
        } else if (sizes.length == 2) {
            frameLayout.setPadding(actualSizes.get(2 - 1), actualSizes.get(1 - 1), actualSizes.get(2 - 1), actualSizes.get(1 - 1));
        } else if (sizes.length == 4) {
            // we are allowing user to specify as t,l,b,r. However, setPadding order is different
            frameLayout.setPadding(actualSizes.get(2 - 1), actualSizes.get(1 - 1), actualSizes.get(4 - 1), actualSizes.get(3 - 1));
        }
    }

    @SimpleProperty(description = "Returns the current Padding sizes")
    ...
    @SimpleProperty(description = "Sets the custom font for title bar. To reset, set fontFileName to empty")
    public void SetTitleBarCustomFont(String fontFileName) {
        this.fontFileName = fontFileName.trim();
    ...
    public void Title(String title) {
        ...
        try {
            s.setSpan(new com.google.appinventor.components.runtime.TypefaceSpan($context(), isRepl()
                            , fontFileName), 0, s.length(),
                    Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);

            // Update the action bar title with the TypefaceSpan instance
            actionBar.setTitle(s);
        } catch (Exception e) {
            Log.d(LOG_TAG, "Unable to set custom font. Revert to no regular font. Error: " + e.getLocalizedMessage());
            actionBar.setTitle(s);
        }
    }

    // http://www.coderzheaven.com/2014/10/09/change-title-subtitle-text-color-actionbar-android/
    @DesignerProperty(editorType = PropertyTypeConstants.PROPERTY_TYPE_COLOR, defaultValue = DEFAULT_VALUE_COLOR_WHITE)
    @SimpleProperty(description = "Sets the color of the ActionBar Title")
    public void TitleColor(int color) {
        this.titleColor = color;
        getActionBar().setTitle(Html.fromHtml("<font color='" + color + "'>" + title + "</font>"));
    }

    @SimpleProperty(description = "Reports the color value of the ActionBar Title")
...
//      System.err.println("Form.setChildWidth(): Setting " + component + " lastwidth to " + width);
    ...
    @DesignerProperty(editorType = PropertyTypeConstants.PROPERTY_TYPE_COLOR,
            defaultValue = Component.DEFAULT_VALUE_COLOR_INDIGO)
...
//            MenuItem item  = menu.add(Menu.NONE, Menu.NONE, mapMenuItems.size(), aMenuItem);
    ...
    private void showAboutApplicationNotification() {
        ...
        Notifier.oneButtonDialog(this, message, title, buttonText, doNothing, 2, false);
    ...
    // this is not being used anymore. Features have been taken into admob, interstatial and rewarded component.
    // Rational: these components use firebase and didn't want form to include firebase.jar

    //method not working; hence hiding it
//  @SimpleProperty(description = "Checks to see if user has granted all required permissions")
    public boolean ArePermissionsGranted() {
        return PermissionUtil.arePermissionsGranted($context());
    }
    // Initially used this method to get ALL permissions from Manifest and 1by1 request the permission from
    // user. searchkey requestAllPermission

    @SimpleFunction(description = "NOTE: THIS BLOCK SHOULD NOT BE USED. PLEASE DO NOT USE. IT WILL BE REMOVED LATER!")
    public void RequestAppPermission() {
        PermissionUtil.checkRuntimePermission($context());
    }
    ...
    public void askPermission(final String permission, final PermissionResultHandler responseRequestor) {
        ...
        androidUIHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                if (ABPermissions.hasPermission($context(), permission)) {
                ...
                ABPermissions.requestPermissions((Activity) form, new String[]{permission}, nonce);
            }
        });
    }
    ...
    @DesignerProperty(editorType = PropertyTypeConstants.PROPERTY_TYPE_MAX_API_LEVEL_OPTIONS, defaultValue = "26")
    @SimpleProperty(userVisible = false, description = "Max API Level")
    public void MaxAPI(String maxAPI) {
        // We don't actually do anything This property is stored in the
        // project properties file
    }
